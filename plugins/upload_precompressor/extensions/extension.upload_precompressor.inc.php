...
{
  rex_register_extension('MEDIA_ADDED', 'upload_precompressor', array()); // REX < 4.5
}
else
{
  rex_register_extension('MEDIA_ADDED', 'upload_precompressor', array(), REX_EXTENSION_EARLY); // REX >= 4.5, use early EP
}


function upload_precompressor($params)
{
  global $REX;
  $scalable_mime_types = array('image/jpeg', 'image/jpg', 'image/pjpeg');
  $mypage = 'upload_precompressor';
  unregister_rex_extension('MEDIA_ADDED', 'upload_precompressor'); // unregister this extension point, it's called twice from REDAXO-Core...

    if (in_array($params['type'], $scalable_mime_types) && $params['ok'])
    {
      // check if image needs scaling
      if ($params['width'] > $REX['ADDON']['upload_precompressor']['settings']['max_pixel'] || $params['height'] > $REX['ADDON']['upload_precompressor']['settings']['max_pixel'])
      {
        if ($params['width'] > $params['height'])
        {
          $ratio = $REX['ADDON']['upload_precompressor']['settings']['max_pixel'] / $params['width'];
        }
        else
        {
          $ratio = $REX['ADDON']['upload_precompressor']['settings']['max_pixel'] / $params['height'];
        }
        ...
        // save Image
        imagejpeg($image, $REX['MEDIAFOLDER'].'/'.$params['filename'], $REX['ADDON']['upload_precompressor']['settings']['jpg_quality']);
